...
const Listing: FC<IListing> = ({ patients, removePatient }) => {
  const PatientCard: FC<IPatientCard> = ({ patient, removePatient }) => {
    return (
      ...
      // </div>
      <div  key={patient.id} className='m-2 inline-block w-80 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700'>
        <div className='flex justify-end px-4 pt-4'>
            ...
            <div id='dropdown' className='hidden z-10 w-10 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700'>
                <ul className='py-1' aria-labelledby='dropdownButton'>
                <li>
                    <a href='#' className='block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white'>✎</a>
                </li>
                <li>
                    <a href='#' className='block py-2 px-4 text-sm text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white'>ⓧ</a>
                </li>
                </ul>
            </div>
        </div>
        <div className='flex flex-col items-center pb-10'>
            <h5 className='mb-1 text-xl font-medium text-gray-900 dark:text-white'>{patient.firstName} {patient.middleName} {patient.lastName}</h5>
            <span className='text-sm text-gray-500 dark:text-gray-400'>Gender : {patient.gender}</span>
            <span className='text-sm text-gray-500 dark:text-gray-400'>Age : {patient.age}</span>
            <span className='text-sm text-gray-500 dark:text-gray-400'>Phone No : {patient.phoneNo}</span>
            <span className='text-sm text-gray-500 dark:text-gray-400'>Vaccination Status : {patient.vaccinationStatus}</span>
            <span className='text-sm text-gray-500 dark:text-gray-400'>Vaccination Name : {patient.vaccineName}</span>
